...
def callback_wp(data, component_instance):
    ...
    logger.debug("Received JointState names: %s on topic %s"%(data.name, self.get_topic_name()))
    logger.debug("Received JointState positons: %s on topic %s"%(data.position, self.get_topic_name()))
    logger.debug("Received JointState velocity: %s on topic %s"%(data.velocity, self.get_topic_name()))
